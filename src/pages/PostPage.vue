  ...
  try {
    ...
    if (response.data.RC === 0) {
      ...
      for (let i = 0; i < arr.length - 1; i += 1) {
        ...
        if (arr[i].conn_to_db === '1') {
          legendName = arr[i + 1].conn_to_db === '-1' ? 'сервер не работвал' : 'всё хорошо'
          lineType = arr[i + 1].conn_to_db === '-1' ? 'dashed' : 'solid'
          lineColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
          barColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
        } else if (arr[i].conn_to_db === '0') {
          legendName = arr[i + 1].conn_to_db === '-1' ? 'сервер не работвал' : 'не было связи с бд'
          lineType = 'dashed'
          lineColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
          barColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
        } else if (arr[i].conn_to_db === '-1') {
          legendName = arr[i + 1].conn_to_db === '-1' ? 'сервер не работвал' : 'всё хорошо'
          lineType = arr[i + 1].conn_to_db === '-1' ? 'dashed' : 'solid'
          lineColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
          barColor = arr[i + 1].conn_to_db === '-1' ? '#8C8C8C' : '#00e5ff'
...
// ГРАФИК !!!!!!!!!!!
// const optionsRangebar = reactive({
//   chart: {
//     locales: [ru],
//     defaultLocale: 'ru'
//   },
